	...
	public class ResLoader {
		...
		public function tryToCloseLoad():void {
			try {
				...
				_urlLoader.close();
				AppCenter.instance.timer.clearTimer(checkLoad);
				_isLoading = false;
			} catch (e:Error) {
			}
		}
		...
		private function doLoad():void {
			_isLoading = true;
			_urlRequest.url = AppCenter.getResPath(_url);
		...
		private function onError(e:Event):void {
			AppCenter.instance.log.error("Load Error:", e.toString());
			endLoad(null);
		}
		...
		public function load(url:String, type:int, complete:Handler, progress:Handler, isCache:Boolean = true):void {
			if (_isLoading) {
				AppCenter.instance.log.warn("Loader is try to close.", _url);
				tryToCloseLoad();
			}
			...
			AppCenter.instance.timer.doLoop(5000, checkLoad);
			doLoad();
		}
		...
		private function checkLoad():void {
			if (_loaded - _lastLoaded < minBytePre5Second) {
				AppCenter.instance.log.warn("load time out:" + _url);
				tryToCloseLoad();
				endLoad(null);
			} else {
				_lastLoaded = _loaded;
